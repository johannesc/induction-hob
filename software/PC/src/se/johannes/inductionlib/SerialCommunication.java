...
public class SerialCommunication implements
    ...
    private int getPowerLevelFromByte(byte data) {
        for (int i = 0; i < powerLevels2Byte.length; i++) {
            if (powerLevels2Byte[i] == (data & ZONE_HOT_MASK)) {
                return i;
            }
        }
        return -1;
     ...
     * 98 xx yy Where xx is the checksum of the PACKET_TYPE_COMMAND yy is the
     * checksum of this PACKET_TYPE_ACK
     */
    ...
    // no
// ack is sent
    static final short POT_PRESENT_STATUS = 0x2C47; // when removing pot from
                                                    // power card
    ...
    static final byte POWER_LEVEL_MASK = 0x1F;
    ...
    private void decodeData() {
        ...
        if (buffer[0] == PACKET_TYPE_COMMAND) {
            ...
            String commandString = String.format("%04X", command);
            String paramString = "";
            switch (command) {
            ...
            case POWERED_ON_COMMAND:
                ...
                if (buffer[4] == (byte) 0x15) {
                    ...
                    if (powerStatus == (byte) 0x00) {
                        paramString = " OFF";
                    } else if (powerStatus == (byte) 0x01) {
                        paramString = " ON (idle)";
                    } else if (powerStatus == (byte) 0x03) {
                    ...
                    byte[] powerLevels = new byte[4];
                    System.arraycopy(buffer, 7, powerLevels, 0, 4);

                    paramString += " LF:" + (powerLevels[0] & POWER_LEVEL_MASK);
                    if ((powerLevels[0] & ZONE_HOT_MASK) != 0) {
                        paramString += "H";
                    }

                    paramString += " LB:" + (powerLevels[1] & POWER_LEVEL_MASK);
                    if ((powerLevels[1] & ZONE_HOT_MASK) != 0) {
                        paramString += "H";
                    }

                    paramString += " RB:" + (powerLevels[2] & POWER_LEVEL_MASK);
                    if ((powerLevels[2] & ZONE_HOT_MASK) != 0) {
                        paramString += "H";
                    }

                    paramString += " RF:" + (powerLevels[3] & POWER_LEVEL_MASK);
                    if ((powerLevels[3] & ZONE_HOT_MASK) != 0) {
                        paramString += "H";
                    }

                    boolean[] hot = new boolean[4];
                    for (int i = 0; i < hot.length; i++) {
                        hot[i] = ((powerLevels[i] & ZONE_HOT_MASK) != 0);
                    }

                    int[] levels = new int[4];
                    for (int i = 0; i < levels.length; i++) {
                        levels[i] = getPowerLevelFromByte(powerLevels[i]);
                    }
                    powerCardCallback.onPoweredOnCommand(powerStatus, levels,
                            hot);
                } else if (buffer[4] == (byte) 0x18) {
                    paramString = " (PLIMIT)"; // Power limited
                    paramString += " LF LB RB RF:" + getHexString(buffer, 7, 4);

                    int[] levels = new int[4];
                    for (int i = 0; i < levels.length; i++) {
                        levels[i] = getPowerLevelFromByte(buffer[i + 7]);
                    }
                    powerCardCallback.onPowerLimitCommand(levels);
                } else {
                    paramString = " -------------------UNKNOWN!:" + buffer[4];
                    powerCardCallback.onUnknownData();
                }
                break;
           ...
           default:
            ...
            System.out.println(getTs() + getHexString(buffer, 0, packetLen));
            System.err.println(getTs() + "ACK package cmd=" + commandString);
            System.err.println();
        } else {
